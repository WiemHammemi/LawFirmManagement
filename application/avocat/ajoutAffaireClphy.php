   ...
   ?>
    ...
    notiDiv.addEventListener('click', function() {
        ...
        <p class="head-1">Ajouter affaire</p>
                <br />
                <form action="ajoutAffaireClphy.php" method="POST" class="payer" style="border:none;">

<input type="text" id="cin" name="cin" required placeholder="Numéro de CIN de Client" ><br><br>
...
<input type="text" id="desc" name="description" required placeholder="Description de l'affaire"><br><br>
...
<input type="submit" name="ajoutAff" value="Ajouter">
        ...
        {
            if(isset($_POST['cin'])&& isset($_POST['datedeb']))
            { $des=$_POST['description'] ;
                $cin=$_POST['cin'];
                $datedeb = date('Y-m-d', strtotime($_POST['datedeb']));
                $d1=mysqli_real_escape_string($conn,$des) ;
               ...
               // $cinhn = isset($_POST['cinhn']) ? $_POST['cinhn'] : '';
                ...
                $sql="SELECT * from client_phy where id_cin_clphy='$cin'";
                ...
                {   
                    ...
                    $sql1="INSERT into affaire (id_cin,id_cin_clphy,commission,descrip,date_dep,confirmHN,confirmEx) values ('" . $_SESSION['id'] . "','$cin','$comm','$d1','$datedeb','non','non')";
                    ...
                    } else  echo"<script> window.onload = function(){alert('Erreur se produite , veuillez ressayer .')}</script>";
                ...
                }else echo"<script> window.onload = function(){alert('Le longuer de numero de CIN du client doit etre égale à 8 !')}</script>";
